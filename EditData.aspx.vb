...
Public Class EditData
    ...
    Private Sub btnCancelDemographic_Click(sender As Object, e As EventArgs) Handles btnCancelDemographic.Click
        ...
        grdContract.DataSource = Nothing
    ...
    Private Sub btnContinueDemographic_Click(sender As Object, e As EventArgs) Handles btnContinueDemographic.Click
        ...
        Dim _History = oRegistery.tblHistories.FirstOrDefault
        If _History IsNot Nothing Then
            grdContract.DataSource = _History.tblContracts.ToList
        Else
            grdContract.DataSource = Nothing
        End If
        grdContract.DataBind()
        ShowModalPopup(Me, "pnlSelectContract")

        hfActivePanelId.Value = "History"
    End Sub
    Private Sub grdContract_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles grdContract.RowDataBound
        If e.Row.DataItem IsNot Nothing Then
            Dim oContract As tblContract = e.Row.DataItem

            Dim hfID As HiddenField = e.Row.FindControl("hfID")
            Dim lblContractDateOf As Label = e.Row.FindControl("lblContractDateOf")
            Dim lblCC As Label = e.Row.FindControl("lblCC")
            Dim lblDesc As Label = e.Row.FindControl("lblDesc")

            hfID.Value = oContract.ID
            lblContractDateOf.Text = Miladi2Shamsi(oContract.ContractDateOf)
            lblCC.Text = oContract.CC
            lblDesc.Text = oContract.FreeText
        End If
    End Sub
    ...
    Private Sub btnOK_Click(sender As Object, e As EventArgs) Handles btnOK.Click
        ...
        If Session("Contract") IsNot Nothing Then
    ...
    Private Function isNotValidDateOf(oNumber As Integer, oNMin As Integer, oNMax As Integer, oDateOf As DateTime?, oDatePart As String) As Boolean
        ...
        Return False
    End Function
    Private Sub btSearchByFileNo_Click(sender As Object, e As EventArgs) Handles btnSearchByFileNo.Click
        If txtFileNo.Text.Trim = "" Then
    ...
    End Sub
    Private Sub btnSearchByNationalCode_Click(sender As Object, e As EventArgs) Handles btnSearchByNationalCode.Click
        If txtNationalCode.Text.Trim = "" Then
            btnCancelDemographic_Click(Nothing, Nothing)
            Exit Sub
        End If
    ...
    Private Function FillDemographic(oFieldName As String) As Boolean
        ...
        ElseIf oFieldName.ToLower = "FileNo".ToLower Then
            pnlDemographicMessage.Visible = True
            Dim _fileNoPartS = txtFileNo.Text.Trim.Split("-")
            lblDemographicMessage.Text = "پرونده‌ای با شماره "
            For idx = _fileNoPartS.Count - 1 To 0 Step -1
                lblDemographicMessage.Text &= _fileNoPartS(idx)
            Next
            lblDemographicMessage.Text &= " یافت نشد."
        ElseIf oFieldName.ToLower = "FileNo".ToLower Then
            pnlDemographicMessage.Visible = True
            lblDemographicMessage.Text = "بیماری با کدملی " & txtFileNo.Text.Trim & " یافت نشد."
        End If
    ...
    Private Sub SaveHistory()
        If Session("History") Is Nothing Then Exit Sub
        Dim odb As New dbDataContext(ConnectionStringDiabeticFoot)
        Dim _historyID As Integer = Session("History").ID

        Dim oHistory = odb.tblHistories.Where(Function(f) f.ID = _historyID).FirstOrDefault
    ...
    Private Sub SaveContract()
        ...
        Else
            Dim _contractID As Integer = Session("Contract").ID
            oContract = odb.tblContracts.Where(Function(f) f.ID = _contractID).FirstOrDefault
        End If
        Dim _str = ""
        With oContract
            'Physical Exam
    ...
    Private Sub grdContract_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles grdContract.RowCommand
        ...
        btnCancelSelectContract_Click(Nothing, Nothing)
    End Sub
    Private Sub FillContractData()
        FillHistory()
        If Session("History") IsNot Nothing Then
            Dim _historyID As Integer = Session("History").ID
            Dim odb As New dbDataContext(ConnectionStringDiabeticFoot)
            Dim _contract = odb.tblContracts.Where(Function(f) f.HistoryID = _historyID).FirstOrDefault
            If _contract IsNot Nothing Then
                Session("Contract") = _contract
                With _contract
                    'Physical Exam
                    txtCC.Text = .CC
                    dpContract.SetFromMiladiValue(.ContractDateOf)
                    If .Sore Then
                        cbxSore02.Checked = True
                        dpNewSore.SetFromMiladiValue(.SoreDateOf)
                        Dim oNow = Getdate()
                        txtDuration08.Text = DateDiff(DateInterval.Month, .SoreDateOf.Value, oNow)
                        txtNewSoreCountOfL.Text = Val(.SoreCountLeft.Value)
                        txtNewSoreCountOfR.Text = Val(.SoreCountRight.Value)
                        txtNewMaxLength.Text = Val(.LargestLength.Value)
                        'If .SoreLocationLeft IsNot Nothing Then

                        'End If
                        Dim _Location = .SoreLocationLeft.Split(",")
                        For Each oitem As ListItem In cblNewSoreL.Items
                            oitem.Selected = _Location.Contains(oitem.Value)
                        Next
                        _Location = .SoreLocationRight.Split(",")
                        For Each oitem As ListItem In cblNewSoreR.Items
                            oitem.Selected = _Location.Contains(oitem.Value)
                        Next
                        If .WorstLeftSoreLU.HasValue Then
                            rblNewWorstSoreL.SelectedIndex = .WorstLeftSoreLU.Value - 1
                        End If
                        If .WorstRightSoreLU.HasValue Then
                            rblNewWorstSoreR.SelectedIndex = .WorstRightSoreLU.Value - 1
                        End If
                    End If
                    If .Infection Then
                        cbxInfect01.Checked = True
                        dpNewInfect.SetFromMiladiValue(.InfectionDateOf)
                        Dim oNow = Getdate()
                        txtDuration09.Text = DateDiff(DateInterval.Month, .InfectionDateOf.Value, oNow)
                        Dim _Location = .InfectionLocationLeft.Split(",")
                        For Each oitem As ListItem In cblNewInfectL.Items
                            oitem.Selected = _Location.Contains(oitem.Value)
                        Next
                        _Location = .InfectionLocationRight.Split(",")
                        For Each oitem As ListItem In cblNewInfectR.Items
                            oitem.Selected = _Location.Contains(oitem.Value)
                        Next
                    End If
                    If .Swell Then
                        cbxSwell01.Checked = True
                        dpNewSwell.SetFromMiladiValue(.SwellDateOf)
                        Dim oNow = Getdate()
                        txtDuration10.Text = DateDiff(DateInterval.Month, .SwellDateOf.Value, oNow)
                        Dim _Location = .SwellLocationLeft.Split(",")
                        For Each oitem As ListItem In cblNewSwellL.Items
                            oitem.Selected = _Location.Contains(oitem.Value)
                        Next
                        _Location = .SwellLocationRight.Split(",")
                        For Each oitem As ListItem In cblNewSwellR.Items
                            oitem.Selected = _Location.Contains(oitem.Value)
                        Next
                    End If
                    Dim _L = .Neuropathy.Split(",")
                    For Each oitem As ListItem In cblNeuropathy.Items
                        oitem.Selected = _L.Contains(oitem.Value)
                    Next
                    If .DryLU.HasValue Then
                        rblDry.SelectedIndex = .DryLU.Value - 1
                    End If
                    If .HeatLU.HasValue Then
                        rblTemp.SelectedIndex = .HeatLU.Value - 1
                    End If

                    'Labtests Result
                    txtFBS.Text = Val(.FBS)
                    txtA1C.Text = Val(.A1C)
                    dpDateOf02.SetFromMiladiValue(.LabDateOf)
                    If .Systol.HasValue Then txtSystol.Text = Val(.Systol)
                    If .Dyastol.HasValue Then txtDyastol.Text = Val(.Dyastol)
                    If .O2.HasValue Then txtO2.Text = Val(.O2)
                    If .HR.HasValue Then txtHR.Text = Val(.HR)
                    If .RR.HasValue Then txtRR.Text = Val(.RR)
                    If .VitalSignDateOf.HasValue Then dpDateOf03.SetFromMiladiValue(.VitalSignDateOf)

                    'Prescription
                    cbxNeedAmp.Checked = .Amputation
                    cbxNeedSurg.Checked = .Surgery
                    cbxNeedDebrid.Checked = .Debrid
                    cbxNeedShoe.Checked = .Shoe
                    cbxNeedVisit.Checked = .Visit
                    cbxNeedCover.Checked = .Cover
                    cbxNeedEducation.Checked = .Consult
                    cbxNeedRehab.Checked = .Rehabilitation
                    Dim _Presc = .Drug.Split(",")
                    For Each oitem As ListItem In cblDrugs.Items
                        oitem.Selected = _Presc.Contains(oitem.Value)
                    Next
                    txtFreeText.Text = .FreeText
                End With
            End If
        End If
    End Sub
    Private Sub FillHistory()
        ...
        hfActivePanelId.Value = "History"
    End Sub
    Private Sub btnCancelSelectContract_Click(sender As Object, e As EventArgs) Handles btnCancelSelectContract.Click
        hfNewPanelVisibility.Value = 0
        pnlSelectContract.Style.Item("display") = "none"
        pnlSelectContractBack.Visible = False
    End Sub
End Class